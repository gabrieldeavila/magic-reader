...
function usePositions({ text = [] }: { text?: IText[] }) {
  ...
  const customMimic = useCallback((customText: IText[]) => {
    return customText.reduce((acc, item) => {
      let words = item.value.split("");
  ...
  const getFirstAndLastNode = useCallback(
    (customText?: IText[]) => {
      ...
      if (firstNode.getAttribute("data-block-id") == null) {
        const lastTextId = textToUse[textToUse.length - 1]?.id;
        const firstTextId = textToUse[0]?.id;
      ...
      if (customText) {
        letters = customMimic(customText);
      }
    ...
    [customMimic, getLinesBetween, mimic, text]
  );
  ...
  const getSelectedBlocks = useCallback(
    (customText?: IText[]) => {
      ...
      const {
        ...
        areFromDiffLines,
        multiLineInfo,
      } = getFirstAndLastNode(customText);

      const selected = selectedLetters?.slice(
        firstNodeIndex,
        lastNodeIndex + 1
      );
      ...
      const selectedBlocks =
        selectedIds == null
          ? []
          : textToUse.filter(({ id }) => selectedIds.includes(id));
